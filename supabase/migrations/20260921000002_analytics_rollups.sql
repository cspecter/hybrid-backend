...
--
-- Raw events plus the source tables are too slow to query per dashboard load, so
-- everything the dashboards read comes from these five tables, one row per subject
-- per day. A pg_cron job refreshes the recent window; the same function backfills
-- history, so there is one code path and not two that can disagree.
--
-- Metrics that come from source tables (follows, likes, stashes, comments,
-- subscriptions, entries, claims) have real history, because those rows carry
-- created_at. Metrics that come from analytics_events (impressions, reach, profile
-- visits, shares, taps, removals) start when tracking went live. That start date is
-- recorded in analytics_config so the dashboards can say so rather than showing a
-- flat zero and letting someone read it as "nobody looked".

create table if not exists public.analytics_daily_profile (
  profile_id              integer not null references public.profiles(id) on delete cascade,
  day                     date not null,
  impressions             integer not null default 0,
  reach_accounts          integer not null default 0,
  reach_sessions          integer not null default 0,
  profile_visits          integer not null default 0,
  followers_gained        integer not null default 0,
  followers_lost          integer not null default 0,
  likes_received          integer not null default 0,
  comments_received       integer not null default 0,
  -- Hybrid's differentiator: stashes that happened because of this profile.
  restashes_driven        integer not null default 0,
  restashes_from_posts    integer not null default 0,
  restashes_from_lists    integer not null default 0,
  restashes_from_profile  integer not null default 0,
  list_subscribers_gained integer not null default 0,
...
create table if not exists public.analytics_daily_post (
  ...
  shares          integer not null default 0,
  stashes_driven  integer not null default 0,
  -- Video. watch_ms_total / watch_events gives the average; completions/views the rate.
...
create table if not exists public.analytics_daily_product (
  ...
  unstashes         integer not null default 0,
  list_adds         integer not null default 0,
  posts_tagging     integer not null default 0,
  primary key (product_id, day)
);

create table if not exists public.analytics_daily_location (
  ...
  deal_claims     integer not null default 0,
  deal_redemptions integer not null default 0,
  primary key (location_id, day)
);

create table if not exists public.analytics_daily_giveaway (
  giveaway_id     integer not null references public.giveaways(id) on delete cascade,
  day             date not null,
  views           integer not null default 0,
  reach_accounts  integer not null default 0,
  entries         integer not null default 0,
  primary key (giveaway_id, day)
);

do $$
declare t text;
begin
  foreach t in array array['analytics_daily_profile','analytics_daily_post','analytics_daily_product',
                           'analytics_daily_location','analytics_daily_giveaway'] loop
    execute format('alter table public.%I enable row level security', t);
    execute format('revoke all on public.%I from anon, authenticated, public', t);
    execute format('grant all on public.%I to service_role', t);
    execute format('create index if not exists %I on public.%I (day)', t || '_day_idx', t);
  end loop;
end $$;

-- ─── The refresh ─────────────────────────────────────────────────────────────
-- Rebuilds every rollup for [p_from, p_to]. Idempotent: it deletes the range and
-- recomputes it, so a re-run after a late-arriving event gives the same answer as a
-- first run. The cron job passes a short recent window; the backfill passes years.
...
begin
  ...
  from (
    ...
    select d.location_id, cd.claimed_at::date, 0,0, 0, 0,0,0, count(*),
      ...
      from public.claimed_deals cd join public.deals d on d.public_id = cd.deal_id
...
-- Nightly, plus a same-day pass every hour so a dashboard opened in the afternoon
-- is not showing yesterday. Follows the two existing jobs: pg_cron calling SQL.
select cron.unschedule('analytics_rollup_recent') where exists (select 1 from cron.job where jobname='analytics_rollup_recent');
select cron.schedule('analytics_rollup_recent', '7 * * * *',
  $$select public.analytics_refresh((now() at time zone 'UTC')::date - 1, (now() at time zone 'UTC')::date)$$);

select cron.unschedule('analytics_rollup_nightly') where exists (select 1 from cron.job where jobname='analytics_rollup_nightly');
select cron.schedule('analytics_rollup_nightly', '25 4 * * *',
  $$select public.analytics_refresh((now() at time zone 'UTC')::date - 7, (now() at time zone 'UTC')::date)$$);
