-- Widen home feed eligibility to include image posts and legacy media rows.
-- The mobile app feed renders any post with a file, but the RPC had been
-- restricted to videos only, which can yield an empty initial feed when the
-- dataset is primarily images or older rows with null resource_type.
